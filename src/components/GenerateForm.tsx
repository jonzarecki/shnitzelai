...
function StepReveal({ delay, children }: { delay: number; children: React.ReactNode }) {
...
export function GenerateForm() {
	...
	async function handlePreview() {
		...
		try {
			...
			const data = (await res.json()) as PreviewData & { error?: string; message?: string };

			if (!res.ok) { setError(data.error ?? `שגיאה ${res.status}`); setPhase("idle"); return; }
			if (data.message) { setError(data.message); setPhase("idle"); return; }
	...
	async function handleGenerate() {
		...
		try {
			const res = await fetch("/api/generate/confirm", {
				...
				body: JSON.stringify({
					...
					headlines: preview.headlines.map((h) => ({
						headline: h.headline, summary: "", source: h.source, url: "", category: "general",
					})),
			...
			const data = (await res.json()) as { generation: GenerationData; error?: string };

			if (!res.ok) { setError(data.error ?? `שגיאה ${res.status}`); setPhase("previewed"); return; }
		...
		<div className="space-y-5">
			...
			{phase === "previewing" && (
				<div className="rounded-xl bg-stone-800 px-6 py-4 text-center">
					<p className="text-lg font-bold text-amber-400 animate-pulse">שולף כותרות ובוחר נושא...</p>
					<p className="mt-1 text-xs text-stone-500">קריאת חדשות → בחירת נושא → כתיבת prompt (10-15 שניות)</p>
				</div>
			)}
			...
			{preview && phase !== "idle" && (
				<div className="space-y-4" key={revealKey}>
					...
					<StepReveal delay={0}>
						{preview.recentTopics.length > 0 ? (
							<div className="rounded-lg border border-stone-800 bg-stone-900/50 px-4 py-3">
								<p className="text-xs font-medium text-stone-400 mb-2">שניצלים קודמים השבוע:</p>
								<ul className="space-y-0.5">
									{preview.recentTopics.map((t, i) => (
										<li key={i} className="text-xs text-stone-500 py-0.5">
											...
											<span className="text-amber-400/60">{t.schnitzelHeadline}</span>
										</li>
									))}
								</ul>
							</div>
							...
							<div className="rounded-lg border border-stone-800 bg-stone-900/50 px-4 py-3">
								<p className="text-xs text-stone-500">אין שניצלים קודמים השבוע — זה הראשון!</p>
							</div>
						)}
					</StepReveal>
					...
					<StepReveal delay={300}>
						<div className="rounded-lg border border-stone-800 bg-stone-900/50 px-4 py-3">
							<p className="text-xs font-medium text-stone-400 mb-2">📰 נשלפו {preview.headlines.length} כותרות מ-Google News</p>
							<details>
								<summary className="cursor-pointer text-xs text-stone-500">הצג כותרות</summary>
								<ul className="max-h-48 overflow-y-auto mt-2 space-y-0.5">
									{preview.headlines.map((h, i) => {
										...
										return (
											<li key={i} className={`text-xs py-0.5 ${isRelated ? "font-bold text-amber-400" : "text-stone-500"}`}>
												<span className="text-stone-600">{i + 1}. [{h.source}]</span> {h.headline}
											</li>
										);
									})}
								</ul>
					...
					<StepReveal delay={600}>
						<div className="rounded-lg border border-amber-500/30 bg-stone-900 p-4 space-y-2">
							<p className="text-xs font-medium text-stone-400">🧠 הנושא שנבחר <span className="text-stone-600">({preview.curate.model})</span></p>
							<p className="text-sm text-stone-200">{preview.curate.theme}</p>
							<p className="text-xs text-stone-500">{preview.curate.reasoning}</p>
						</div>
					</StepReveal>
					...
					<StepReveal delay={900}>
						<div className="rounded-lg bg-amber-500/10 border border-amber-500/20 px-4 py-3 text-center">
							<p className="text-xl font-bold text-amber-400">{preview.curate.tagline}</p>
						</div>
					</StepReveal>

					{/* Step E: Image prompt */}
					<StepReveal delay={1200}>
						<div className="rounded-lg border border-stone-700 bg-stone-900 p-4 space-y-3">
							<p className="text-xs font-medium text-stone-400">🎨 הפרומפט לתמונה <span className="text-stone-600">({preview.imagePrompt.model})</span></p>

							<div className="rounded-md bg-stone-800 border border-stone-700 p-3">
								<p className="text-xs font-medium text-stone-300 mb-1">מה תהיה בתמונה:</p>
								<p className="text-sm text-stone-400 leading-relaxed">{preview.imagePrompt.essence}</p>
							</div>

							<details>
								<summary className="cursor-pointer text-xs text-stone-500">הפרומפט המלא</summary>
								<p className="mt-2 text-xs leading-relaxed text-stone-500 font-mono bg-stone-800 rounded-md p-3 max-h-48 overflow-y-auto" dir="ltr">
						...
						<>
							...
							{phase === "generating" && (
								<div className="rounded-xl bg-stone-800 border border-green-500/30 px-6 py-4 text-center">
									<p className="text-lg font-bold text-green-400 animate-pulse">🍳 מייצר תמונה...</p>
									<p className="mt-1 text-xs text-stone-500">יצירת תמונה לוקחת 30-60 שניות</p>
								</div>
							)}
							...
							{generation && (
								<div className="rounded-lg border border-green-500/30 bg-stone-900 overflow-hidden">
									...
									<div className="p-4 text-center">
										<p className="text-lg font-bold text-amber-400">{generation.schnitzel_headline}</p>
									</div>
								</div>
							)}
